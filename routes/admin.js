...
const validaPostagem = require('../control/validaPostagem.js')
...
router.post('/postagem/edit', eAdmin, (req,res) =>{
    ...
    let erros = validaPostagem(req.body)
    if(erros.length > 0){
...
router.get('/mensagens', eAdmin, (req, res) =>{
    Mensagem.find().lean().then((mensagens) =>{
        res.render('admin/mensagens', {mensagens: mensagens})
    }).catch((err) =>{
        req.flash('error_msg', 'Houve um erro ao listas as mensagens!')
    })
})

router.post('/mensagens/deletar', eAdmin, (req, res) =>{
    Mensagem.deleteOne({_id: req.body.id}).then(() =>{
        req.flash('success_msg', 'Mensagem deletada com sucesso!')
        res.redirect('/admin/mensagens')
    }).catch((err) =>{
        req.flash('error_msg', 'Houve um erro ao deletar a mensagem!')
        res.redirect('/admin/mensagens')
    })
})
